...
class WanGenerate:
    def __init__(self, args: argparse.Namespace):
        ...
        self.dit_low_noise_model = self.load_dit_model(self.args.dit, self.dit_weight_dtype, "cpu", self.dit_weight_dtype)
        end_time = time.time()
        print(f"load dit low noise model to cpu time: {end_time - start_time} seconds")

        start_time = time.time()
        # high noise 先被使用，加载到 CPU 上
        self.dit_high_noise_model = self.load_dit_model(self.args.dit_high_noise, self.dit_weight_dtype, "cpu", self.dit_weight_dtype)
        end_time = time.time()
        ...
        self.scheduler = setup_scheduler(self.args, self.gen_settings.cfg, self.gen_settings.device)

    
    def load_vae(self, loading_device):
        vae = WanVAE(vae_path=self.args.vae, device=loading_device, dtype=self.vae_dtype, cache_device=None)
        return vae


    def load_t5(self, loading_device):
        checkpoint_path = None if self.args.ckpt_dir is None else os.path.join(self.args.ckpt_dir, self.config.t5_checkpoint)
        tokenizer_path = None if self.args.ckpt_dir is None else os.path.join(self.args.ckpt_dir, self.config.t5_tokenizer)
    ...
    def load_dit_model(self, dit_path, dit_weight_dtype, loading_device, loading_weight_dtype, lora_weights_list=None, lora_multipliers=None, is_from_image=False):
        # 仅加载 dit 模型
...
class WanGenerateImageClass(WanGenerate):
    def __init__(self, args: argparse.Namespace):
        super().__init__(args)
        self.n_prompt = args.negative_prompt if args.negative_prompt else self.config.sample_neg_prompt

    
    def prepare_inputs(self, seed, encoded_context):
        noise, inputs = prepare_t2v_inputs(self.args, self.cfg, self.accelerator, self.device, self.vae, encoded_context)
        return noise, inputs
    ...
    def generate_single_image(self, prompt, seed):
        ...
        for i, t in enumerate(tqdm(timesteps)):
            ...
            is_high_noise = (t / 1000.0) >= self.args.timestep_boundary if self.args.timestep_boundary is not None else False
            ...
            if not is_high_noise and prev_high_noise:
                ...
                if len(self.models) > 1 and (self.args.offload_inactive_dit or self.args.lazy_loading):
                    ...
                    if self.args.offload_inactive_dit:
                        ...
                        if self.args.blocks_to_swap > 0:
                            # prepare block swap for low noise model
                            self.models[-1].move_to_device_except_swap_blocks(self.device)
            ...
            with self.accelerator.autocast(), torch.no_grad():
                noise_pred_cond = model(latent_model_input, t=timestep, **arg_c)[0].to(self.device)
                ...
                if apply_cfg:
                    ...
                    if self.args.slg_mode == "original" and apply_slg:
                        noise_pred_uncond = model(latent_model_input, t=timestep, **inputs)[0].to(self.device)
                        ...
                        skip_layer_out = model(latent_model_input, t=timestep, skip_block_indices=self.args.slg_layers, **inputs)[0].to(
                            self.device
                        )

                        # apply skip layer guidance
                        # SD3 formula: scaled = scaled + (pos_out - skip_layer_out) * self.slg
                        noise_pred = noise_pred + self.args.slg_scale * (noise_pred_cond - skip_layer_out)
                    elif self.args.slg_mode == "uncond" and apply_slg:
                        # noise_pred_uncond is skip layer out
                        noise_pred_uncond = model(latent_model_input, t=timestep, skip_block_indices=self.args.slg_layers, **inputs)[0].to(
                            self.device
                        )
                        ...
                        # normal guidance
                        noise_pred_uncond = model(latent_model_input, t=timestep, **arg_null)[0].to(self.device)
    ...
    def generate_seeds_images(self, prompt: str, prompt_index: int):
        ...
        for seed in seeds:
            ...
            self.save_image(images, self.args.save_path, save_name)
    ...
    def decode_latent(self, latent):
        ...
        with torch.autocast(device_type=self.device.type, dtype=self.vae.dtype), torch.no_grad():
...
def save_i1mages(sample: torch.Tensor, args: argparse.Namespace, original_base_name: Optional[str] = None) -> str:
...
if __name__ == "__main__":
    ...
    # print(args.device, args.negative_prompt); exit(-1)
